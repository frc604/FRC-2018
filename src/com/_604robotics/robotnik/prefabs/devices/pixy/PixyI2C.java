...
public class PixyI2C {
	String name;
	PixyPacketI2C values;
	I2C pixy;
	Port port = Port.kOnboard;
	PixyPacketI2C[] packets;
	PixyException pExc;
	String print;

	public PixyI2C(String id, I2C argPixy, PixyPacketI2C[] argPixyPacket, PixyException argPixyException,
			PixyPacketI2C argValues) {
		pixy = argPixy;
	...
	public PixyPacketI2C readPacket(int Signature) throws PixyException {
		int Checksum;
		int Sig;
		...
		for (int i = 0; i <= 16; i++) {
			...
			if (syncWord == 0xaa55) { // Check is first 2 bytes equal a "sync
				...
				packets[Sig - 1] = new PixyPacketI2C();
		...
		PixyPacketI2C pkt = packets[Signature - 1];
	...
	private PixyPacketI2C readBlock(int checksum) {
		...
		PixyPacketI2C block = new PixyPacketI2C();
	...
	public PixyPacketI2C[] readBlocks() {
		// This has to match the max block setting in pixymon?
		int maxBlocks = 2;
		PixyPacketI2C[] blocks = new PixyPacketI2C[maxBlocks];
